-- JO Academy community modes
-- Safe to run more than once in Supabase SQL Editor.

-- Allow controlled maintenance updates from Supabase SQL Editor.
-- Browser/API callers still go through RLS and have a non-null auth.uid().
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.community_settings
      ADD CONSTRAINT community_settings_mode_check CHECK (mode IN ('posts', 'chat'));
  END IF;
END $$;
